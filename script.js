// script.js — Reproductor YouTube + Media Session + Notificaciones + Service Worker messaging
document.addEventListener('DOMContentLoaded', function () {
  ...
  const CONFIG = {
    ...
    API_KEYS: [
      ...
      "AIzaSyBr2nxeKaN1q07fMV59zrLEOQx9dzZBsMI",
  ...
  let videoShown = true;
  let transferInProgress = false; // evita transferencias concurrentes
  ...
  function loadAndPlayById(videoId, autoplay=true, setQueue=true, indexOverride=null){
    ...
    ensureNotificationPermission().then(ok => { if(ok) showMediaNotification(currentTrack, autoplay); });
  }
  ...
  /* =====================
     ...
     ===================== */
  function getThumb(track){ return track?.thumb || track?.thumbnail || 'https://dummyimage.com/600x600/ddd/222'; }
  ...
  function renderRecents(){
    ...
    recList.forEach((tr, idx)=>{
      ...
      el.addEventListener('click', (ev)=>{ if(ev.target.closest('.btn-add')||ev.target.closest('.btn-fav')||ev.target.closest('.btn-remove')) return; queue = recList.slice(); currentIndex = idx; currentTrack = queue[currentIndex]; playCurrent(); });
  ...
  function renderFavorites(){
    ...
    favList.forEach((tr, idx)=>{
      ...
      el.addEventListener('click', (ev)=>{ if(ev.target.closest('.btn-add')||ev.target.closest('.btn-fav')||ev.target.closest('.btn-remove')) return; queue = favList.slice(); currentIndex = idx; currentTrack = queue[currentIndex]; playCurrent(); });
      el.querySelector('.btn-add').addEventListener('click', (ev)=>{ ev.stopPropagation(); openAddToPlaylistModal(tr); });
  ...
  /* ==========================
     ...
     ========================== */
  function miniToBack(){ const m = document.getElementById('miniPlayer'); if(m) m.classList.add('behind'); }
  function miniToFront(){ const m = document.getElementById('miniPlayer'); if(m) m.classList.remove('behind'); }
  ...
  function setSuggestionIndex(i){ suggestionIndex = i; highlightSuggestion(); }
  ...
  if(searchInputEl){
    ...
    searchInputEl.addEventListener('keydown', (e)=>{
      ...
      if(visible){
        ...
        if(e.key === 'Enter'){
          e.preventDefault();
          if(suggestionIndex >= 0){ selectSuggestionByIndex(suggestionIndex); }
          else { onSearch(true); }
          return;
        }
        if(e.key === 'Escape'){ clearSuggestions(); return; }
      } else {
        if(e.key === 'Enter'){ e.preventDefault(); onSearch(true); return; }
      }
    });
  ...
  $('#shuffleBtn').addEventListener('click', ()=>{ shuffle = !shuffle; updateShuffleButtonUI(); showToast('Aleatorio: '+(shuffle?'ON':'OFF')); });
  $('#repeatBtn').addEventListener('click', ()=>{ repeatMode = repeatMode === 'none' ? 'all' : (repeatMode === 'all' ? 'one' : 'none'); updateRepeatButtonUI(); showToast('Repetir: '+repeatMode); });
  ...
  async function onSearch(clearInput = false){
    ...
    try{
      ...
      lastResults = res || [];
      if(res && res.length > 0) renderResults(res);
      else $('#listArea').innerHTML = `<div style="padding:32px;text-align:center;color:var(--muted)">No se encontraron resultados</div>`;
    }catch(e){
      ...
      clearSuggestions();
      if(clearInput){
        try{ const inp = $('#searchInput'); if(inp){ inp.value = ''; inp.blur(); } }catch(e){}
      }
    }
  }
  ...
  window._mp = {STORAGE, saveStorage, queue}; // debugging

  /* =====================
     Visibility handling: robust transfer
     ===================== */
        ...
        try{
          if(visiblePlayer && typeof visiblePlayer.getPlayerState === 'function' &&
            ...
            if(ok){
              ...
              // no hacemos stop en visible para evitar corte
            }
          }
...
/* =====================
   Registro del Service Worker (fuera de DOMContentLoaded)
   ===================== */
if ('serviceWorker' in navigator) {
